...
public class WavFile extends AudioFile {
	...
	private double[] convertToCanonicalForm(double[] data) {
		...
		double duplicatingFactorOrig = (double)CANONICAL_SAMPLING_RATE / sampleRate;		
		...
		if (duplicatingFactorOrig == 0.91875) {
			...
			for (int i = 0; i < newlen; i++) {
				...
				double slope = (data[nearestRightPosition] - data[nearestLeftPosition]);
				double positionFromLeft = currentPosition - nearestLeftPosition;
				newFileSamples[i] = ((slope * positionFromLeft) + data[nearestLeftPosition]);
			}
		} else {
